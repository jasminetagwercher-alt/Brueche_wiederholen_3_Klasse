...
function addOperands(difficulty){
  ...
  if(difficulty===3){return[randomReducedFraction({minDen:3,maxDen:9}),randomReducedFraction({minDen:3,maxDen:9})]}
  for(let i=0;i<40;i++){const a=randomReducedFraction({minDen:3,maxDen:10}),b=randomReducedFraction({minDen:3,maxDen:10}),r=a.add(b);if(r.numerator>r.denominator&&cleanResult(r))return[a,b]}
  return[F(5,6),F(3,4)];
}
...
function subtractOperands(difficulty){
  for(let i=0;i<60;i++){
    ...
    if(a.compare(b)>0&&cleanResult(a.subtract(b)))return[a,b];
  }
  return[F(5,6),F(1,4)];
}
