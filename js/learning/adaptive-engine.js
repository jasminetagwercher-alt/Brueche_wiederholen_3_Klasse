export function chooseTrainingSkill(skillState,recentSkills=[]){
  const entries=Object.values(skillState).filter(s=>!['mixed_practice','error_analysis','plausibility'].includes(s.id));
  const weighted=[];
...
export function phaseSkillSequence(phase,skillState,length){
  ...
  if(phase==='mix')return ['compare','add','fraction_of_quantity','multiply','number_line','decimal_to_fraction','subtract','mixed_numbers','divide','order'].slice(0,length);
