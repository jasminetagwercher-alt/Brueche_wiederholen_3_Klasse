...
export function overviewTemplate(session,phaseMeta){
  ...
  const cards=ORDER.slice(0,4).map((phase,index)=>{
    const state=index<current?'Abgeschlossen':index===current?'Du bist hier':'Kommt später';
  ...
  return `<section class="card">${phaseRoadmap(session.phase)}<div class="section-heading"><p class="eyebrow">Deine Lernstrecke</p><h1>Wo du gerade stehst</h1><p>Die Strecke ist bewusst in kurze Abschnitte geteilt. Du musst nicht alles am Stück erledigen.</p></div><div class="overview-list">${cards}</div><div class="actions"><button id="backToTask">Zur aktuellen Aufgabe</button></div></section>`;
}

export function breakTemplate({session,phaseMeta,completed,total,phaseIndependent}){
  const meta=phaseMeta[session.phase];
  return `<section class="card break-card">${phaseRoadmap(session.phase)}<div class="break-icon" aria-hidden="true">Ⅱ</div><p class="eyebrow">Kurze Pause</p><h1>${completed} von ${total} Aufgaben geschafft</h1><p>Dieser Abschnitt ist erledigt. Schau für einen Moment weg vom Bildschirm, lockere Hände und Schultern und mache weiter, wenn du bereit bist.</p><div class="break-stats"><div><strong>${phaseIndependent}</strong><span>davon ohne Hilfe</span></div><div><strong>${total-completed}</strong><span>Aufgaben in diesem Abschnitt übrig</span></div></div><div class="actions"><button id="continueAfterBreak">Weiter mit ${meta.shortTitle||meta.title}</button><button id="breakOverview" class="secondary">Übersicht</button></div></section>`;
}
